...
4. [sourceModel.test.ts](./sourceModel.test.ts) - Contains the untrusted source envelope contract
...
This directory owns the untrusted source-local execution contract schema, including subject, fact, relation, export-summary, registry-source collection, and source-envelope models, plus later canonical normalization, strict parsing, local closure validation, resource budgets, and source digest. Qualified and compiled contracts belong to the later SC02B slice, while namespace qualification, module-signature validation, evidence admission, and compiler diagnostics belong to SC03.

`implementation.ts` is the package-local facade. Keep subject, fact, relation, export summary, registry source collection, source envelope, budget accounting, canonical parsing, semantic parsing, and source closure concerns in focused internal modules.
