...
describe("module graph snapshot", () => {
  ...
  it("exports the producer API from the transformer root", () => {
    expect(publicApi.canonicalizeModuleUrl).toBe(canonicalizeModuleUrl);
    expect(publicApi.createModuleGraphSnapshot).toBe(createModuleGraphSnapshot);
    expect(publicApi.parseModuleGraphSnapshot).toBe(parseModuleGraphSnapshot);
    expect(publicApi.ModuleGraphError).toBe(ModuleGraphError);
  });
});
