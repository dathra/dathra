...
#adr(
  ...
  [
    ...
    - `source-content-id` identifies the normalized source snapshot contents.
      Equal normalized source may intentionally have the same
      `source-content-id` across
      different instances.
...
#adr(
  ...
  [
    ...
    - The activation artifact must bind its expected `activation-instance-id` and
      expected `source-content-id` to the handoff record. The client verifies both bindings
      before registering listener, state, or timer ownership.
    - A missing, duplicate, stale, or mismatched `activation-instance-id`,
      `source-content-id`,
...
#adr(
  ...
  [
    ...
    - A handoff record has no cross-route, cross-request, or global registry
      lifetime. It is not reusable by a later instance merely because its
      `source-content-id` matches.
  ...
  [
    - The empty source is a valid normalized source and follows the same record
      contract.
    - Multiple active blocks retain independent records even when their
      `source-content-id` values are equal.
  ...
  alternatives: [
    - *Use a global source registry keyed by `source-content-id`*: This introduces
...
#adr(
  ...
  [
    ...
    - Each `activation-root` has one logical handoff table mapping its local
      `activation-instance-id` values to the exact `normalized-source` strings
      produced by #105.
...
#behavior_spec(
  ...
  preconditions: [
    ...
    - the root marker, host marker, artifact admission, and root-scoped handoff
      table have matching `activation-root` and `activation-instance-id` values
  ],
  postconditions: [
...
#behavior_spec(
  ...
  preconditions: [
    - an expected `activation-root`, `activation-instance-id`, artifact
      identity, marker, or handoff record is missing, stale, duplicated, or
      mismatched
  ],
  postconditions: [
